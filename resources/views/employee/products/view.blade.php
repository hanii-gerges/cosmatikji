...
<div class="panel panel-default view_users" style="margin: 10px;">
    ...
    <table class="table">
        <tr class="h4 text-center">
            <td style="width:2%;">#</td>
            <td style="width:10%;">اسم الصنف الفرعي</td>
            ...
            <td>{{$product->name}}</td>
            <td>{{$product->price}}</td>
            <td><a href="{{route('emp.show.multipic',$product->id)}}">صور المنتج اضغط هنا</a></td>
            <td>{{$product->description}}</td>
            <td class="text-center">
                <a href="{{route('edit.product',$product->id)}}" class="glyphicon glyphicon-pencil text-info"></a>
                <a href="{{route('delete.product',$product->id)}}" class="glyphicon glyphicon-trash text-danger" onclick="return confirm('هل انتا متأكد من انك تريد حذف المنتج');"></a>
            </td>
        </tr>
            @endforeach
