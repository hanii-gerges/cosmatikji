...
<div class="page_content">
    ...
    <h1 class="heading_title">تعديل حساب الموظف</h1>
    ...
    <div class="form">
        <form class="form-horizontal" action="{{route('employee.update.profile')}}" method="POST" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                <label for="input0" class="col-sm-2 control-label bring_right left_text">اسم العضو</label>
                <div class="col-sm-10">
                    <input type="text" class="form-control" placeholder="اسم المدير" value="{{$employee->name}}" name="name">
                </div>
            </div>
            <div class="form-group">
                <label for="input2" class="col-sm-2 control-label bring_right left_text">البريد الالكتروني</label>
                <div class="col-sm-10">
                    <input type="email" class="form-control"placeholder="البريد الالكتروني" value="{{$employee->email}}" name="email">
                </div>
            </div>
            {{-- <div class="form-group">
